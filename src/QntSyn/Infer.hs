...
inferBindings :: [(Identifier, Expr)] -> Infer TypeEnv
inferBindings bs =
  let f env group = M.union env <$> localTE (M.union env) (inferBindingGroup group)
  in foldM f M.empty $ makeBindingGroups bs
...
inferTopLevel :: [TopLevel] -> InferEnv -> Either TypeError TypeEnv
inferTopLevel xs env =
  let sigs    = filter (\x -> case x of {TLTypeSig _ _ -> True; _ -> False}) xs -- TODO: Handle these
      assigns = filter (\x -> case x of {TLAssign  _ _ -> True; _ -> False}) xs
      assigns' = (\(TLAssign ident expr) -> (ident, expr)) <$> assigns
  in runInfer (inferBindings assigns') env >>= \(tyEnv, cs) ->
     if not $ null cs
     then Left $ CannotSolveError cs
     else Right tyEnv
